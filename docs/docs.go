// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "termsOfService": "http://swagger.io/terms/",
        "contact": {
            "name": "API Support",
            "email": "support@example.com"
        },
        "license": {
            "name": "MIT",
            "url": "https://opensource.org/licenses/MIT"
        },
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/api/v1/cam/iam/audit/logs": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "审计日志管理"
                ],
                "summary": "查询审计日志列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "操作类型",
                        "name": "operation_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "操作人ID",
                        "name": "operator_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "目标类型",
                        "name": "target_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "云平台",
                        "name": "cloud_platform",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "开始时间",
                        "name": "start_time",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束时间",
                        "name": "end_time",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.PageResult"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/audit/logs/export": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "审计日志管理"
                ],
                "summary": "导出审计日志",
                "parameters": [
                    {
                        "type": "string",
                        "description": "操作类型",
                        "name": "operation_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "操作人ID",
                        "name": "operator_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "目标类型",
                        "name": "target_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "云平台",
                        "name": "cloud_platform",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "开始时间",
                        "name": "start_time",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "结束时间",
                        "name": "end_time",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "导出格式(csv/json)",
                        "name": "format",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "file"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/audit/reports": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "审计日志管理"
                ],
                "summary": "生成审计报告",
                "parameters": [
                    {
                        "description": "生成审计报告请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.GenerateAuditReportVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/groups": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "查询用户组列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "关键词",
                        "name": "keyword",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.PageResult"
                        }
                    }
                }
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "创建用户组",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "创建用户组请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateUserGroupVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/groups/sync": {
            "post": {
                "description": "从云平台同步用户组信息，包括用户组基本信息、权限策略和用户组成员。支持阿里云、腾讯云等多云平台。\n同步结果包含：用户组统计（总数、新建、更新、失败）和成员统计（总数、同步成功、失败）",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "同步云平台用户组及其成员",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "cloud_account_id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "同步结果统计",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/web.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/service.GroupSyncResult"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "参数错误",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    },
                    "500": {
                        "description": "同步失败",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/groups/{id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "获取用户组详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户组ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "更新用户组信息",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户组ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新用户组请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateUserGroupVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "删除用户组",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户组ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/groups/{id}/members": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "获取用户组成员列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户组ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/groups/{id}/policies": {
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户组管理"
                ],
                "summary": "更新用户组的权限策略",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户组ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新权限策略请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdatePoliciesVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/permissions/groups/{group_id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限管理"
                ],
                "summary": "获取用户组的所有权限",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "用户组ID",
                        "name": "group_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/permissions/policies": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限管理"
                ],
                "summary": "按云厂商查询可用的权限策略",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "cloud_account_id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/permissions/users/{user_id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限管理"
                ],
                "summary": "获取用户的所有权限",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "用户ID",
                        "name": "user_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/permissions/users/{user_id}/effective": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "权限管理"
                ],
                "summary": "获取用户的有效权限（包含用户组继承的权限）",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "用户ID",
                        "name": "user_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/sync/tasks": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "同步任务管理"
                ],
                "summary": "查询同步任务列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务类型",
                        "name": "task_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "cloud_account_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.PageResult"
                        }
                    }
                }
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "同步任务管理"
                ],
                "summary": "创建同步任务",
                "parameters": [
                    {
                        "description": "创建同步任务请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateSyncTaskVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/sync/tasks/{id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "同步任务管理"
                ],
                "summary": "获取同步任务状态",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/sync/tasks/{id}/retry": {
            "post": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "同步任务管理"
                ],
                "summary": "重试失败的同步任务",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/templates": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "策略模板管理"
                ],
                "summary": "查询策略模板列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "模板分类",
                        "name": "category",
                        "in": "query"
                    },
                    {
                        "type": "boolean",
                        "description": "是否内置",
                        "name": "is_built_in",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "关键词",
                        "name": "keyword",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.PageResult"
                        }
                    }
                }
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "策略模板管理"
                ],
                "summary": "创建策略模板",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "创建策略模板请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateTemplateVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/templates/from-group": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "策略模板管理"
                ],
                "summary": "从用户组创建策略模板",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "从用户组创建模板请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateFromGroupVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/templates/{id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "策略模板管理"
                ],
                "summary": "获取策略模板详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "模板ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "策略模板管理"
                ],
                "summary": "更新策略模板信息",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "模板ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新策略模板请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateTemplateVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "策略模板管理"
                ],
                "summary": "删除策略模板",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "模板ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/tenants": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "租户管理"
                ],
                "summary": "查询租户列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "关键词",
                        "name": "keyword",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "行业",
                        "name": "industry",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地区",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.PageResult"
                        }
                    }
                }
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "租户管理"
                ],
                "summary": "创建租户",
                "parameters": [
                    {
                        "description": "创建租户请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateTenantVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/tenants/{tenant_id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "租户管理"
                ],
                "summary": "获取租户详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "租户管理"
                ],
                "summary": "更新租户信息",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新租户请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateTenantVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "租户管理"
                ],
                "summary": "删除租户",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/tenants/{tenant_id}/stats": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "租户管理"
                ],
                "summary": "获取租户统计信息",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/users": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "查询用户列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "用户类型",
                        "name": "user_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "页码",
                        "name": "page",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "每页数量",
                        "name": "size",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.PageResult"
                        }
                    }
                }
            },
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "创建云用户",
                "parameters": [
                    {
                        "description": "创建用户请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateUserVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/users/batch-assign": {
            "post": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "批量分配用户组",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "批量分配用户组请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.AssignUserGroupsVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/users/sync": {
            "post": {
                "description": "创建异步同步任务，立即返回任务ID，可通过任务ID查询同步进度",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "同步云平台用户（异步）",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "cloud_account_id",
                        "in": "query",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/web.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.SyncTask"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/iam/users/{id}": {
            "get": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "获取用户详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "put": {
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "更新用户信息",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新用户请求",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateUserVO"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            },
            "delete": {
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "用户管理"
                ],
                "summary": "删除用户",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "用户ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/menus": {
            "get": {
                "description": "获取系统导航菜单的层级结构",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "获取菜单列表",
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/bindings/{id}": {
            "delete": {
                "tags": [
                    "服务树"
                ],
                "summary": "解绑资源",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "绑定ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/environments": {
            "get": {
                "tags": [
                    "服务树-环境"
                ],
                "summary": "获取环境列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.EnvironmentVO"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "post": {
                "tags": [
                    "服务树-环境"
                ],
                "summary": "创建环境",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "环境信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateEnvReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "integer",
                                            "format": "int64"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/environments/init": {
            "post": {
                "tags": [
                    "服务树-环境"
                ],
                "summary": "初始化默认环境",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/environments/{id}": {
            "get": {
                "tags": [
                    "服务树-环境"
                ],
                "summary": "获取环境详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "环境ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.EnvironmentVO"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "put": {
                "tags": [
                    "服务树-环境"
                ],
                "summary": "更新环境",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "环境ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "环境信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateEnvReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "tags": [
                    "服务树-环境"
                ],
                "summary": "删除环境",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "环境ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/nodes": {
            "get": {
                "tags": [
                    "服务树"
                ],
                "summary": "获取节点列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "父节点ID",
                        "name": "parent_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "层级",
                        "name": "level",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "名称",
                        "name": "name",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.NodeVO"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "post": {
                "tags": [
                    "服务树"
                ],
                "summary": "创建服务树节点",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "节点信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateNodeReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "integer",
                                            "format": "int64"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/nodes/{id}": {
            "get": {
                "tags": [
                    "服务树"
                ],
                "summary": "获取节点详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.NodeVO"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "put": {
                "tags": [
                    "服务树"
                ],
                "summary": "更新节点",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "节点信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateNodeReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "tags": [
                    "服务树"
                ],
                "summary": "删除节点",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/nodes/{id}/bindings": {
            "get": {
                "tags": [
                    "服务树"
                ],
                "summary": "获取节点绑定的资源",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "环境ID",
                        "name": "env_id",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.BindingVO"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "post": {
                "tags": [
                    "服务树"
                ],
                "summary": "绑定资源到节点",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "资源信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.BindResourceReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "integer",
                                            "format": "int64"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/nodes/{id}/bindings/batch": {
            "post": {
                "tags": [
                    "服务树"
                ],
                "summary": "批量绑定资源",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "资源列表",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.BatchBindReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "integer",
                                            "format": "int64"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/nodes/{id}/move": {
            "put": {
                "tags": [
                    "服务树"
                ],
                "summary": "移动节点",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "目标父节点",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.MoveNodeReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/resources/{type}/{id}/node": {
            "get": {
                "tags": [
                    "服务树"
                ],
                "summary": "获取资源所属节点",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资源类型",
                        "name": "type",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "资源ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.NodeVO"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/rules": {
            "get": {
                "tags": [
                    "服务树"
                ],
                "summary": "获取规则列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "节点ID",
                        "name": "node_id",
                        "in": "query"
                    },
                    {
                        "type": "boolean",
                        "description": "是否启用",
                        "name": "enabled",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.RuleVO"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "post": {
                "tags": [
                    "服务树"
                ],
                "summary": "创建绑定规则",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "description": "规则信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateRuleReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "integer",
                                            "format": "int64"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/rules/execute": {
            "post": {
                "description": "手动触发规则引擎，对未绑定资源执行自动匹配，绑定到规则指定的环境",
                "tags": [
                    "服务树"
                ],
                "summary": "执行规则匹配",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "integer",
                                            "format": "int64"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/rules/{id}": {
            "get": {
                "tags": [
                    "服务树"
                ],
                "summary": "获取规则详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.RuleVO"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "put": {
                "tags": [
                    "服务树"
                ],
                "summary": "更新规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "规则信息",
                        "name": "body",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateRuleReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "tags": [
                    "服务树"
                ],
                "summary": "删除规则",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "规则ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/api/v1/cam/service-tree/tree": {
            "get": {
                "tags": [
                    "服务树"
                ],
                "summary": "获取服务树结构",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "根节点ID",
                        "name": "root_id",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.TreeNodeVO"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/cam/assets": {
            "get": {
                "description": "获取云资产列表，支持按云厂商、资产类型、状态等条件过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理"
                ],
                "summary": "获取资产列表",
                "parameters": [
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "azure"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "ecs",
                            "rds",
                            "oss",
                            "vpc"
                        ],
                        "type": "string",
                        "description": "资产类型",
                        "name": "asset_type",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "running",
                            "stopped",
                            "deleted"
                        ],
                        "type": "string",
                        "description": "资产状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.CloudAsset"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "post": {
                "description": "创建新的云资产记录",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理"
                ],
                "summary": "创建资产",
                "parameters": [
                    {
                        "description": "资产信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateAssetReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.CloudAsset"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/assets/batch": {
            "post": {
                "description": "批量创建多个云资产记录",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理"
                ],
                "summary": "批量创建资产",
                "parameters": [
                    {
                        "description": "批量资产信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateMultiAssetsReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.CloudAsset"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/assets/cost-analysis": {
            "get": {
                "description": "获取云资产的成本分析报告",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "统计分析"
                ],
                "summary": "获取成本分析",
                "parameters": [
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "azure"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 30,
                        "description": "分析天数",
                        "name": "days",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/assets/discover": {
            "post": {
                "description": "从指定云厂商和地域发现新的云资产",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产发现"
                ],
                "summary": "发现云资产",
                "parameters": [
                    {
                        "description": "发现资产请求",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.DiscoverAssetsReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.CloudAsset"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/assets/ecs": {
            "get": {
                "description": "从数据库获取已同步的云虚拟机实例列表，支持按IP地址过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-ECS"
                ],
                "summary": "获取云虚拟机列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "内网IP",
                        "name": "private_ip",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "公网IP",
                        "name": "public_ip",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "VPC ID",
                        "name": "vpc_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/ecs/{asset_id}": {
            "get": {
                "description": "从数据库获取指定云虚拟机实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-ECS"
                ],
                "summary": "获取云虚拟机详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(云厂商实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/eip": {
            "get": {
                "description": "从数据库获取已同步的弹性公网IP列表，支持按绑定实例类型、IP地址、VPC等过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-EIP"
                ],
                "summary": "获取弹性公网IP列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态(InUse/Available)",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "EIP名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "IP地址(精确匹配)",
                        "name": "ip_address",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "绑定的实例ID",
                        "name": "instance_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "绑定的实例类型(EcsInstance/SlbInstance/Nat/HaVip/NetworkInterface)",
                        "name": "instance_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "VPC ID",
                        "name": "vpc_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "线路类型(BGP/BGP_PRO/ChinaTelecom/ChinaUnicom/ChinaMobile)",
                        "name": "isp",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "绑定状态: bound(已绑定)/unbound(未绑定)",
                        "name": "bindable",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/eip/{asset_id}": {
            "get": {
                "description": "从数据库获取指定弹性公网IP的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-EIP"
                ],
                "summary": "获取弹性公网IP详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(EIP Allocation ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "EIP不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/elasticsearch": {
            "get": {
                "description": "从数据库获取已同步的Elasticsearch实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-Elasticsearch"
                ],
                "summary": "获取Elasticsearch实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "ES版本",
                        "name": "version",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "VPC ID",
                        "name": "vpc_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/elasticsearch/{asset_id}": {
            "get": {
                "description": "从数据库获取指定Elasticsearch实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-Elasticsearch"
                ],
                "summary": "获取Elasticsearch实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/kafka": {
            "get": {
                "description": "从数据库获取已同步的Kafka实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-Kafka"
                ],
                "summary": "获取Kafka实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "VPC ID",
                        "name": "vpc_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/kafka/{asset_id}": {
            "get": {
                "description": "从数据库获取指定Kafka实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-Kafka"
                ],
                "summary": "获取Kafka实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/mongodb": {
            "get": {
                "description": "从数据库获取已同步的MongoDB实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-MongoDB"
                ],
                "summary": "获取MongoDB实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/mongodb/{asset_id}": {
            "get": {
                "description": "从数据库获取指定MongoDB实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-MongoDB"
                ],
                "summary": "获取MongoDB实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(云厂商实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/nas": {
            "get": {
                "description": "从数据库获取已同步的NAS文件系统列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-NAS"
                ],
                "summary": "获取NAS文件系统列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件系统名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "文件系统类型(standard/extreme/cpfs)",
                        "name": "file_system_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "协议类型(NFS/SMB)",
                        "name": "protocol_type",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/nas/{asset_id}": {
            "get": {
                "description": "从数据库获取指定NAS文件系统的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-NAS"
                ],
                "summary": "获取NAS文件系统详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(文件系统ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "文件系统不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/oss": {
            "get": {
                "description": "从数据库获取已同步的OSS存储桶列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-OSS"
                ],
                "summary": "获取OSS存储桶列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "存储桶名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "存储类型(Standard/IA/Archive)",
                        "name": "storage_class",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "访问权限(private/public-read/public-read-write)",
                        "name": "acl",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/oss/{asset_id}": {
            "get": {
                "description": "从数据库获取指定OSS存储桶的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-OSS"
                ],
                "summary": "获取OSS存储桶详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(存储桶名称)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "存储桶不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/rds": {
            "get": {
                "description": "从数据库获取已同步的RDS实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-RDS"
                ],
                "summary": "获取RDS实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/rds/{asset_id}": {
            "get": {
                "description": "从数据库获取指定RDS实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-RDS"
                ],
                "summary": "获取RDS实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(云厂商实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/redis": {
            "get": {
                "description": "从数据库获取已同步的Redis实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-Redis"
                ],
                "summary": "获取Redis实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/redis/{asset_id}": {
            "get": {
                "description": "从数据库获取指定Redis实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-Redis"
                ],
                "summary": "获取Redis实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(云厂商实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/search": {
            "get": {
                "description": "跨资产类型搜索，支持按关键词匹配资产ID、名称、IP地址等。返回匹配信息供前端高亮显示",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-搜索"
                ],
                "summary": "统一搜索资产",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "搜索关键词(匹配资产ID、名称、IP等)",
                        "name": "keyword",
                        "in": "query",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产类型(逗号分隔: ecs,rds,redis,mongodb,vpc,eip)",
                        "name": "types",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.SearchResultResponse"
                        }
                    },
                    "400": {
                        "description": "参数错误",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/statistics": {
            "get": {
                "description": "获取云资产的统计信息，包括数量、类型分布等",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "统计分析"
                ],
                "summary": "获取资产统计",
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/assets/sync": {
            "post": {
                "description": "同步指定云账号的资产状态和信息。此接口已废弃，请使用 POST /api/v1/cam/cloud-accounts/{id}/sync",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产发现"
                ],
                "summary": "同步云资产（已废弃）",
                "deprecated": true,
                "parameters": [
                    {
                        "description": "同步资产请求",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.SyncAssetsReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/assets/vpc": {
            "get": {
                "description": "从数据库获取已同步的VPC列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-VPC"
                ],
                "summary": "获取VPC列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "VPC名称(模糊搜索)",
                        "name": "name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetListResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/vpc/{asset_id}": {
            "get": {
                "description": "从数据库获取指定VPC的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理-VPC"
                ],
                "summary": "获取VPC详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "X-Tenant-ID",
                        "in": "header",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "资产ID(VPC ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/web.AssetDetailResult"
                        }
                    },
                    "400": {
                        "description": "租户ID不能为空",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "VPC不存在",
                        "schema": {
                            "$ref": "#/definitions/web.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/cam/assets/{id}": {
            "get": {
                "description": "根据资产ID获取详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理"
                ],
                "summary": "获取资产详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.CloudAsset"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "资产不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "put": {
                "description": "更新指定ID的云资产信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理"
                ],
                "summary": "更新资产",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新的资产信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateAssetReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.CloudAsset"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "资产不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "description": "删除指定ID的云资产记录",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产管理"
                ],
                "summary": "删除资产",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "资产ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "资产不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/cloud-accounts": {
            "get": {
                "description": "获取云账号列表，支持按云厂商、环境、状态等条件过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号管理"
                ],
                "summary": "获取云账号列表",
                "parameters": [
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "azure"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "dev",
                            "test",
                            "prod"
                        ],
                        "type": "string",
                        "description": "环境",
                        "name": "environment",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "active",
                            "inactive"
                        ],
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudAccount"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "post": {
                "description": "创建新的云账号配置",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号管理"
                ],
                "summary": "创建云账号",
                "parameters": [
                    {
                        "description": "云账号信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateCloudAccountReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudAccount"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/cloud-accounts/{id}": {
            "get": {
                "description": "根据ID获取云账号的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号管理"
                ],
                "summary": "获取云账号详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.CloudAccount"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "put": {
                "description": "更新指定ID的云账号信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号管理"
                ],
                "summary": "更新云账号",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新的云账号信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateCloudAccountReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.CloudAccount"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "description": "删除指定ID的云账号",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号管理"
                ],
                "summary": "删除云账号",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/cloud-accounts/{id}/disable": {
            "post": {
                "description": "禁用指定的云账号",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号操作"
                ],
                "summary": "禁用云账号",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/cloud-accounts/{id}/enable": {
            "post": {
                "description": "启用指定的云账号",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号操作"
                ],
                "summary": "启用云账号",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/cloud-accounts/{id}/sync": {
            "post": {
                "description": "同步指定云账号下的云资产到本地数据库。这是同步云资产的主要接口，支持按资源类型和地域过滤同步范围。\n同步过程会自动获取云账号配置的所有地域，并逐个地域同步指定类型的资产。\n如果不指定 asset_types，默认只同步 ECS 实例。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号操作"
                ],
                "summary": "同步云账号资产（推荐）",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "同步请求参数",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.SyncAccountReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "同步成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.SyncResult"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "409": {
                        "description": "云账号已禁用",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/cloud-accounts/{id}/test-connection": {
            "post": {
                "description": "测试指定云账号的连接状态",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "云账号操作"
                ],
                "summary": "测试云账号连接",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "连接成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "云账号不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "连接失败",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/databases/mongodb": {
            "get": {
                "description": "从数据库获取已同步的MongoDB实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "数据库管理"
                ],
                "summary": "获取MongoDB实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "asset_name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.DatabaseInstanceListResp"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/cam/databases/mongodb/{asset_id}": {
            "get": {
                "description": "从数据库获取指定MongoDB实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "数据库管理"
                ],
                "summary": "获取MongoDB实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "资产ID(云厂商实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.DatabaseInstanceVO"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/databases/rds": {
            "get": {
                "description": "从数据库获取已同步的RDS实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "数据库管理"
                ],
                "summary": "获取RDS实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "asset_name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.DatabaseInstanceListResp"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/cam/databases/rds/{asset_id}": {
            "get": {
                "description": "从数据库获取指定RDS实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "数据库管理"
                ],
                "summary": "获取RDS实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "资产ID(云厂商实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.DatabaseInstanceVO"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/databases/redis": {
            "get": {
                "description": "从数据库获取已同步的Redis实例列表",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "数据库管理"
                ],
                "summary": "获取Redis实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例名称(模糊搜索)",
                        "name": "asset_name",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.DatabaseInstanceListResp"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/cam/databases/redis/{asset_id}": {
            "get": {
                "description": "从数据库获取指定Redis实例的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "数据库管理"
                ],
                "summary": "获取Redis实例详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "资产ID(云厂商实例ID)",
                        "name": "asset_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "huawei",
                            "tencent",
                            "volcano"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.DatabaseInstanceVO"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "实例不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/field-groups/{id}": {
            "put": {
                "description": "更新指定ID的字段分组信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段分组管理"
                ],
                "summary": "更新字段分组",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "分组ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新的分组信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateFieldGroupReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "分组不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "description": "删除指定ID的字段分组",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段分组管理"
                ],
                "summary": "删除字段分组",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "分组ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "分组不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/fields/{field_uid}": {
            "put": {
                "description": "更新指定字段的定义信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段管理"
                ],
                "summary": "更新模型字段",
                "parameters": [
                    {
                        "type": "string",
                        "description": "字段UID",
                        "name": "field_uid",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新的字段信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.UpdateFieldReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "字段不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "description": "删除指定的模型字段定义",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段管理"
                ],
                "summary": "删除模型字段",
                "parameters": [
                    {
                        "type": "string",
                        "description": "字段UID",
                        "name": "field_uid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "字段不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/instances": {
            "get": {
                "description": "获取资产实例列表，支持按模型、租户、云账号等条件过滤，支持任意属性组合过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "获取资产实例列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "model_uid",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "租户ID",
                        "name": "tenant_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "云账号ID",
                        "name": "account_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "资产名称(模糊搜索)",
                        "name": "asset_name",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "资产ID(精确匹配)",
                        "name": "asset_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "云平台(aliyun/aws/huawei/tencent/volcengine)",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "地域",
                        "name": "region",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "可用区",
                        "name": "zone",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "VPC ID",
                        "name": "vpc_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "实例规格",
                        "name": "instance_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "操作系统类型",
                        "name": "os_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "计费类型",
                        "name": "charge_type",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "内网IP",
                        "name": "private_ip",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "公网IP",
                        "name": "public_ip",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "是否有标签(true/false)",
                        "name": "has_tags",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "标签键(过滤包含此标签键的实例)",
                        "name": "tag_key",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "标签值(需配合tag_key使用)",
                        "name": "tag_value",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.InstanceListResp"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "post": {
                "description": "创建新的资产实例",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "创建资产实例",
                "parameters": [
                    {
                        "description": "实例信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.CreateInstanceReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/cam/instances/batch": {
            "post": {
                "description": "批量创建多个资产实例",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "批量创建资产实例",
                "parameters": [
                    {
                        "description": "实例列表",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.CreateBatchInstanceReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "object"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/cam/instances/upsert": {
            "post": {
                "description": "根据 tenant_id + model_uid + asset_id 更新或插入实例",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "更新或插入资产实例",
                "parameters": [
                    {
                        "description": "实例信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.UpsertInstanceReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/instances/upsert-batch": {
            "post": {
                "description": "批量根据 tenant_id + model_uid + asset_id 更新或插入实例",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "批量更新或插入资产实例",
                "parameters": [
                    {
                        "description": "实例列表",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.UpsertBatchInstanceReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/instances/{id}": {
            "get": {
                "description": "根据ID获取资产实例详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "获取资产实例详情",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "实例ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.InstanceVO"
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            },
            "put": {
                "description": "更新指定ID的资产实例信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "更新资产实例",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "实例ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.UpdateInstanceReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "description": "删除指定ID的资产实例",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "资产实例"
                ],
                "summary": "删除资产实例",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "实例ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/menus": {
            "get": {
                "description": "获取系统导航菜单的层级结构",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "系统管理"
                ],
                "summary": "获取菜单列表",
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/web.MenuItem"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    }
                }
            }
        },
        "/cam/models": {
            "get": {
                "description": "获取资产模型列表，支持按云厂商、分类等条件过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "模型管理"
                ],
                "summary": "获取模型列表",
                "parameters": [
                    {
                        "enum": [
                            "aliyun",
                            "aws",
                            "azure"
                        ],
                        "type": "string",
                        "description": "云厂商",
                        "name": "provider",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "模型分类",
                        "name": "category",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "父模型UID",
                        "name": "parent_uid",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "模型层级",
                        "name": "level",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_domain.Model"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "post": {
                "description": "创建新的资产模型定义",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "模型管理"
                ],
                "summary": "创建模型",
                "parameters": [
                    {
                        "description": "模型信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.CreateModelReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_domain.Model"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/models/{uid}": {
            "get": {
                "description": "根据模型UID获取模型的详细信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "模型管理"
                ],
                "summary": "获取模型详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_domain.Model"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "模型不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "put": {
                "description": "更新指定UID的模型信息",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "模型管理"
                ],
                "summary": "更新模型",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "更新的模型信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.UpdateModelReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_domain.Model"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "模型不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "description": "删除指定UID的模型",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "模型管理"
                ],
                "summary": "删除模型",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "模型不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/models/{uid}/field-groups": {
            "get": {
                "description": "获取指定模型的所有字段分组",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段分组管理"
                ],
                "summary": "获取字段分组列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/domain.ModelFieldGroup"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "post": {
                "description": "为指定模型添加新的字段分组",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段分组管理"
                ],
                "summary": "添加字段分组",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "字段分组信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateFieldGroupReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/domain.ModelFieldGroup"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/models/{uid}/fields": {
            "get": {
                "description": "获取指定模型的所有字段定义",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段管理"
                ],
                "summary": "获取模型字段列表",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "type": "array",
                                            "items": {
                                                "$ref": "#/definitions/domain.ModelField"
                                            }
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "post": {
                "description": "为指定模型添加新的字段定义",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "字段管理"
                ],
                "summary": "添加模型字段",
                "parameters": [
                    {
                        "type": "string",
                        "description": "模型UID",
                        "name": "uid",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "字段信息",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.CreateFieldReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/domain.ModelField"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/tasks": {
            "get": {
                "description": "获取异步任务列表，支持按任务类型、状态、创建者等条件过滤",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "异步任务"
                ],
                "summary": "获取任务列表",
                "parameters": [
                    {
                        "enum": [
                            "sync_assets",
                            "discover_assets"
                        ],
                        "type": "string",
                        "description": "任务类型",
                        "name": "type",
                        "in": "query"
                    },
                    {
                        "enum": [
                            "pending",
                            "running",
                            "completed",
                            "failed",
                            "cancelled"
                        ],
                        "type": "string",
                        "description": "任务状态",
                        "name": "status",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "创建者",
                        "name": "created_by",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "偏移量",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 20,
                        "description": "限制数量",
                        "name": "limit",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.TaskListResp"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/tasks/discover-assets": {
            "post": {
                "description": "提交异步发现资产任务，从指定云厂商和地域发现新资产",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "异步任务"
                ],
                "summary": "提交发现资产任务",
                "parameters": [
                    {
                        "description": "发现任务参数",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.SubmitDiscoverAssetsTaskReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务提交成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.SubmitTaskResp"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/tasks/sync-assets": {
            "post": {
                "description": "提交异步同步资产任务，支持指定云厂商、资产类型和地域",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "异步任务"
                ],
                "summary": "提交同步资产任务",
                "parameters": [
                    {
                        "description": "同步任务参数",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/web.SubmitSyncAssetsTaskReq"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "任务提交成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.SubmitTaskResp"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "400": {
                        "description": "请求参数错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/tasks/{id}": {
            "get": {
                "description": "根据任务ID获取异步任务的详细信息和执行状态",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "异步任务"
                ],
                "summary": "获取任务详情",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "成功",
                        "schema": {
                            "allOf": [
                                {
                                    "$ref": "#/definitions/ginx.Result"
                                },
                                {
                                    "type": "object",
                                    "properties": {
                                        "data": {
                                            "$ref": "#/definitions/web.TaskResp"
                                        }
                                    }
                                }
                            ]
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            },
            "delete": {
                "description": "删除已完成或已取消的异步任务记录",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "异步任务"
                ],
                "summary": "删除任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "删除成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "任务无法删除",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        },
        "/cam/tasks/{id}/cancel": {
            "post": {
                "description": "取消正在执行或等待执行的异步任务",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "异步任务"
                ],
                "summary": "取消任务",
                "parameters": [
                    {
                        "type": "string",
                        "description": "任务ID",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "取消成功",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "400": {
                        "description": "任务无法取消",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "404": {
                        "description": "任务不存在",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    },
                    "500": {
                        "description": "服务器错误",
                        "schema": {
                            "$ref": "#/definitions/ginx.Result"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "domain.CloudUserStatus": {
            "type": "string",
            "enum": [
                "active",
                "inactive",
                "deleted"
            ],
            "x-enum-varnames": [
                "CloudUserStatusActive",
                "CloudUserStatusInactive",
                "CloudUserStatusDeleted"
            ]
        },
        "domain.CloudUserType": {
            "type": "string",
            "enum": [
                "api_key",
                "access_key",
                "ram_user",
                "iam_user",
                "cam_user",
                "volc_user"
            ],
            "x-enum-comments": {
                "CloudUserTypeCAMUser": "腾讯云 CAM 用户",
                "CloudUserTypeIAMUser": "AWS/华为云 IAM 用户",
                "CloudUserTypeRAMUser": "阿里云 RAM 用户",
                "CloudUserTypeVolcUser": "火山云用户"
            },
            "x-enum-descriptions": [
                "",
                "",
                "阿里云 RAM 用户",
                "AWS/华为云 IAM 用户",
                "腾讯云 CAM 用户",
                "火山云用户"
            ],
            "x-enum-varnames": [
                "CloudUserTypeAPIKey",
                "CloudUserTypeAccessKey",
                "CloudUserTypeRAMUser",
                "CloudUserTypeIAMUser",
                "CloudUserTypeCAMUser",
                "CloudUserTypeVolcUser"
            ]
        },
        "domain.ModelField": {
            "type": "object",
            "properties": {
                "createTime": {
                    "description": "创建时间",
                    "type": "string"
                },
                "display": {
                    "description": "是否显示（兼容旧系统）",
                    "type": "boolean"
                },
                "displayName": {
                    "description": "显示名称（用于前端展示）",
                    "type": "string"
                },
                "fieldName": {
                    "description": "字段名称",
                    "type": "string"
                },
                "fieldType": {
                    "description": "字段类型",
                    "type": "string"
                },
                "fieldUID": {
                    "description": "字段唯一标识",
                    "type": "string"
                },
                "groupID": {
                    "description": "所属分组ID",
                    "type": "integer",
                    "format": "int64"
                },
                "id": {
                    "description": "业务ID",
                    "type": "integer",
                    "format": "int64"
                },
                "index": {
                    "description": "排序索引",
                    "type": "integer"
                },
                "link": {
                    "description": "是否为关联字段（兼容旧系统）",
                    "type": "boolean"
                },
                "linkModel": {
                    "description": "关联模型UID",
                    "type": "string"
                },
                "modelUID": {
                    "description": "所属模型UID",
                    "type": "string"
                },
                "option": {
                    "description": "字段选项（JSON）",
                    "type": "string"
                },
                "required": {
                    "description": "是否必填",
                    "type": "boolean"
                },
                "secure": {
                    "description": "是否敏感字段",
                    "type": "boolean"
                },
                "updateTime": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "domain.ModelFieldGroup": {
            "type": "object",
            "properties": {
                "createTime": {
                    "description": "创建时间",
                    "type": "string"
                },
                "id": {
                    "description": "业务ID",
                    "type": "integer",
                    "format": "int64"
                },
                "index": {
                    "description": "排序索引",
                    "type": "integer"
                },
                "modelUID": {
                    "description": "所属模型UID",
                    "type": "string"
                },
                "name": {
                    "description": "分组名称",
                    "type": "string"
                },
                "updateTime": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "domain.PermissionPolicy": {
            "type": "object",
            "properties": {
                "policy_document": {
                    "type": "string"
                },
                "policy_id": {
                    "type": "string"
                },
                "policy_name": {
                    "type": "string"
                },
                "policy_type": {
                    "$ref": "#/definitions/domain.PolicyType"
                },
                "provider": {
                    "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                }
            }
        },
        "domain.PolicyType": {
            "type": "string",
            "enum": [
                "system",
                "custom"
            ],
            "x-enum-varnames": [
                "PolicyTypeSystem",
                "PolicyTypeCustom"
            ]
        },
        "domain.RuleCondition": {
            "type": "object",
            "properties": {
                "field": {
                    "description": "匹配字段 (provider/region/tag.xxx/name/attributes.xxx)",
                    "type": "string"
                },
                "operator": {
                    "description": "操作符 (eq/ne/contains/regex/in)",
                    "type": "string"
                },
                "value": {
                    "description": "匹配值",
                    "type": "string"
                }
            }
        },
        "domain.SyncTargetType": {
            "type": "string",
            "enum": [
                "user",
                "group",
                "account"
            ],
            "x-enum-comments": {
                "SyncTargetTypeAccount": "云账号级别同步"
            },
            "x-enum-descriptions": [
                "",
                "",
                "云账号级别同步"
            ],
            "x-enum-varnames": [
                "SyncTargetTypeUser",
                "SyncTargetTypeGroup",
                "SyncTargetTypeAccount"
            ]
        },
        "domain.SyncTaskType": {
            "type": "string",
            "enum": [
                "user_sync",
                "permission_sync",
                "group_sync",
                "batch_user_sync"
            ],
            "x-enum-comments": {
                "SyncTaskTypeBatchUserSync": "批量用户同步（从云平台拉取所有用户）"
            },
            "x-enum-descriptions": [
                "",
                "",
                "",
                "批量用户同步（从云平台拉取所有用户）"
            ],
            "x-enum-varnames": [
                "SyncTaskTypeUserSync",
                "SyncTaskTypePermissionSync",
                "SyncTaskTypeGroupSync",
                "SyncTaskTypeBatchUserSync"
            ]
        },
        "domain.TemplateCategory": {
            "type": "string",
            "enum": [
                "read_only",
                "admin",
                "developer",
                "custom"
            ],
            "x-enum-varnames": [
                "TemplateCategoryReadOnly",
                "TemplateCategoryAdmin",
                "TemplateCategoryDeveloper",
                "TemplateCategoryCustom"
            ]
        },
        "domain.TenantMetadata": {
            "type": "object",
            "properties": {
                "cloud_account_count": {
                    "type": "integer"
                },
                "company_name": {
                    "type": "string"
                },
                "contact_email": {
                    "type": "string"
                },
                "contact_phone": {
                    "type": "string"
                },
                "industry": {
                    "type": "string"
                },
                "owner": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                },
                "tags": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "user_count": {
                    "type": "integer"
                },
                "user_group_count": {
                    "type": "integer"
                }
            }
        },
        "domain.TenantSettings": {
            "type": "object",
            "properties": {
                "allowed_providers": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                    }
                },
                "custom_fields": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "string"
                    }
                },
                "features": {
                    "type": "object",
                    "additionalProperties": {
                        "type": "boolean"
                    }
                },
                "max_cloud_accounts": {
                    "type": "integer"
                },
                "max_user_groups": {
                    "type": "integer"
                },
                "max_users": {
                    "type": "integer"
                }
            }
        },
        "domain.TenantStatus": {
            "type": "string",
            "enum": [
                "active",
                "inactive",
                "suspended",
                "deleted"
            ],
            "x-enum-varnames": [
                "TenantStatusActive",
                "TenantStatusInactive",
                "TenantStatusSuspended",
                "TenantStatusDeleted"
            ]
        },
        "ginx.Result": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer"
                },
                "data": {},
                "msg": {
                    "type": "string"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_domain.Model": {
            "type": "object",
            "properties": {
                "category": {
                    "description": "资源类别",
                    "type": "string"
                },
                "createTime": {
                    "description": "创建时间",
                    "type": "string"
                },
                "description": {
                    "description": "描述",
                    "type": "string"
                },
                "extensible": {
                    "description": "是否可扩展",
                    "type": "boolean"
                },
                "icon": {
                    "description": "图标",
                    "type": "string"
                },
                "id": {
                    "description": "业务ID",
                    "type": "integer",
                    "format": "int64"
                },
                "level": {
                    "description": "层级（1=主资源，2=子资源）",
                    "type": "integer"
                },
                "modelGroupID": {
                    "description": "模型分组ID",
                    "type": "integer",
                    "format": "int64"
                },
                "name": {
                    "description": "模型名称",
                    "type": "string"
                },
                "parentUID": {
                    "description": "父模型UID（用于层级关系）",
                    "type": "string"
                },
                "provider": {
                    "description": "云厂商（all表示通用）",
                    "type": "string"
                },
                "uid": {
                    "description": "模型唯一标识",
                    "type": "string"
                },
                "updateTime": {
                    "description": "更新时间",
                    "type": "string"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.CreateBatchInstanceReq": {
            "type": "object",
            "required": [
                "instances"
            ],
            "properties": {
                "instances": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.CreateInstanceReq"
                    }
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.CreateInstanceReq": {
            "type": "object",
            "required": [
                "asset_id",
                "model_uid",
                "tenant_id"
            ],
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "attributes": {
                    "type": "object",
                    "additionalProperties": true
                },
                "model_uid": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.CreateModelReq": {
            "type": "object",
            "required": [
                "category",
                "level",
                "name",
                "provider",
                "uid"
            ],
            "properties": {
                "category": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "extensible": {
                    "type": "boolean"
                },
                "icon": {
                    "type": "string"
                },
                "level": {
                    "type": "integer",
                    "minimum": 1
                },
                "model_group_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "parent_uid": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                },
                "uid": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.InstanceListResp": {
            "type": "object",
            "properties": {
                "instances": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.InstanceVO"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.InstanceVO": {
            "type": "object",
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "attributes": {
                    "type": "object",
                    "additionalProperties": true
                },
                "create_time": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "model_uid": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.UpdateInstanceReq": {
            "type": "object",
            "properties": {
                "asset_name": {
                    "type": "string"
                },
                "attributes": {
                    "type": "object",
                    "additionalProperties": true
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.UpdateModelReq": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "extensible": {
                    "type": "boolean"
                },
                "icon": {
                    "type": "string"
                },
                "model_group_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.UpsertBatchInstanceReq": {
            "type": "object",
            "required": [
                "instances"
            ],
            "properties": {
                "instances": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_cam_web.UpsertInstanceReq"
                    }
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_cam_web.UpsertInstanceReq": {
            "type": "object",
            "required": [
                "asset_id",
                "model_uid",
                "tenant_id"
            ],
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "attributes": {
                    "type": "object",
                    "additionalProperties": true
                },
                "model_uid": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudAccount": {
            "type": "object",
            "properties": {
                "access_key_id": {
                    "description": "访问密钥ID",
                    "type": "string"
                },
                "access_key_secret": {
                    "description": "访问密钥Secret (加密存储)",
                    "type": "string"
                },
                "asset_count": {
                    "description": "资产数量",
                    "type": "integer"
                },
                "config": {
                    "description": "配置信息",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudAccountConfig"
                        }
                    ]
                },
                "create_time": {
                    "description": "创建时间",
                    "type": "string"
                },
                "ctime": {
                    "description": "创建时间戳",
                    "type": "integer"
                },
                "description": {
                    "description": "描述信息",
                    "type": "string"
                },
                "environment": {
                    "description": "环境",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.Environment"
                        }
                    ]
                },
                "error_message": {
                    "description": "错误信息",
                    "type": "string"
                },
                "id": {
                    "description": "账号ID",
                    "type": "integer"
                },
                "last_sync_time": {
                    "description": "最后同步时间",
                    "type": "string"
                },
                "last_test_time": {
                    "description": "最后测试时间",
                    "type": "string"
                },
                "name": {
                    "description": "账号名称",
                    "type": "string"
                },
                "provider": {
                    "description": "云厂商",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                        }
                    ]
                },
                "regions": {
                    "description": "支持的地域列表",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "status": {
                    "description": "账号状态",
                    "allOf": [
                        {
                            "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudAccountStatus"
                        }
                    ]
                },
                "tenant_id": {
                    "description": "租户ID",
                    "type": "string"
                },
                "update_time": {
                    "description": "更新时间",
                    "type": "string"
                },
                "utime": {
                    "description": "更新时间戳",
                    "type": "integer"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudAccountConfig": {
            "type": "object",
            "properties": {
                "enable_auto_sync": {
                    "description": "是否启用自动同步",
                    "type": "boolean"
                },
                "enable_cost_monitoring": {
                    "description": "启用成本监控",
                    "type": "boolean"
                },
                "read_only": {
                    "description": "只读权限",
                    "type": "boolean"
                },
                "show_sub_accounts": {
                    "description": "显示子账号",
                    "type": "boolean"
                },
                "supported_asset_types": {
                    "description": "支持的资产类型",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "supported_regions": {
                    "description": "支持的地域列表",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "sync_interval": {
                    "description": "同步间隔(分钟)",
                    "type": "integer"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudAccountStatus": {
            "type": "string",
            "enum": [
                "active",
                "disabled",
                "error",
                "testing"
            ],
            "x-enum-comments": {
                "CloudAccountStatusActive": "活跃状态",
                "CloudAccountStatusDisabled": "已禁用",
                "CloudAccountStatusError": "错误状态",
                "CloudAccountStatusTesting": "测试中"
            },
            "x-enum-descriptions": [
                "活跃状态",
                "已禁用",
                "错误状态",
                "测试中"
            ],
            "x-enum-varnames": [
                "CloudAccountStatusActive",
                "CloudAccountStatusDisabled",
                "CloudAccountStatusError",
                "CloudAccountStatusTesting"
            ]
        },
        "github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider": {
            "type": "string",
            "enum": [
                "aliyun",
                "aws",
                "azure",
                "tencent",
                "huawei",
                "volcano",
                "volcengine"
            ],
            "x-enum-comments": {
                "CloudProviderAWS": "Amazon Web Services",
                "CloudProviderAliyun": "阿里云",
                "CloudProviderAzure": "Microsoft Azure",
                "CloudProviderHuawei": "华为云",
                "CloudProviderTencent": "腾讯云",
                "CloudProviderVolcano": "火山云 (别名)",
                "CloudProviderVolcengine": "火山引擎"
            },
            "x-enum-descriptions": [
                "阿里云",
                "Amazon Web Services",
                "Microsoft Azure",
                "腾讯云",
                "华为云",
                "火山云 (别名)",
                "火山引擎"
            ],
            "x-enum-varnames": [
                "CloudProviderAliyun",
                "CloudProviderAWS",
                "CloudProviderAzure",
                "CloudProviderTencent",
                "CloudProviderHuawei",
                "CloudProviderVolcano",
                "CloudProviderVolcengine"
            ]
        },
        "github_com_Havens-blog_e-cam-service_internal_shared_domain.Environment": {
            "type": "string",
            "enum": [
                "production",
                "staging",
                "development"
            ],
            "x-enum-comments": {
                "EnvironmentDevelopment": "开发环境",
                "EnvironmentProduction": "生产环境",
                "EnvironmentStaging": "预发环境"
            },
            "x-enum-descriptions": [
                "生产环境",
                "预发环境",
                "开发环境"
            ],
            "x-enum-varnames": [
                "EnvironmentProduction",
                "EnvironmentStaging",
                "EnvironmentDevelopment"
            ]
        },
        "github_com_Havens-blog_e-cam-service_internal_shared_domain.SyncTask": {
            "type": "object",
            "properties": {
                "cloud_account_id": {
                    "type": "integer"
                },
                "create_time": {
                    "type": "string"
                },
                "ctime": {
                    "type": "integer"
                },
                "end_time": {
                    "type": "string"
                },
                "error_message": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "max_retries": {
                    "type": "integer"
                },
                "progress": {
                    "type": "integer"
                },
                "provider": {
                    "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                },
                "retry_count": {
                    "type": "integer"
                },
                "start_time": {
                    "type": "string"
                },
                "status": {
                    "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.SyncTaskStatus"
                },
                "target_id": {
                    "type": "integer"
                },
                "target_type": {
                    "$ref": "#/definitions/domain.SyncTargetType"
                },
                "task_type": {
                    "$ref": "#/definitions/domain.SyncTaskType"
                },
                "update_time": {
                    "type": "string"
                },
                "utime": {
                    "type": "integer"
                }
            }
        },
        "github_com_Havens-blog_e-cam-service_internal_shared_domain.SyncTaskStatus": {
            "type": "string",
            "enum": [
                "pending",
                "running",
                "success",
                "failed",
                "retrying"
            ],
            "x-enum-varnames": [
                "SyncTaskStatusPending",
                "SyncTaskStatusRunning",
                "SyncTaskStatusSuccess",
                "SyncTaskStatusFailed",
                "SyncTaskStatusRetrying"
            ]
        },
        "service.GroupSyncResult": {
            "type": "object",
            "properties": {
                "created_groups": {
                    "type": "integer"
                },
                "failed_groups": {
                    "type": "integer"
                },
                "failed_members": {
                    "type": "integer"
                },
                "synced_members": {
                    "type": "integer"
                },
                "total_groups": {
                    "type": "integer"
                },
                "total_members": {
                    "type": "integer"
                },
                "updated_groups": {
                    "type": "integer"
                }
            }
        },
        "web.AssetDetailResult": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer",
                    "example": 0
                },
                "data": {
                    "$ref": "#/definitions/web.UnifiedAssetVO"
                },
                "msg": {
                    "type": "string",
                    "example": "success"
                }
            }
        },
        "web.AssetListResult": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer",
                    "example": 0
                },
                "data": {
                    "$ref": "#/definitions/web.UnifiedAssetListResp"
                },
                "msg": {
                    "type": "string",
                    "example": "success"
                }
            }
        },
        "web.AssignUserGroupsVO": {
            "type": "object",
            "required": [
                "group_ids",
                "user_ids"
            ],
            "properties": {
                "group_ids": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "integer"
                    }
                },
                "user_ids": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "integer"
                    }
                }
            }
        },
        "web.BatchBindReq": {
            "type": "object",
            "required": [
                "env_id",
                "resource_ids",
                "resource_type"
            ],
            "properties": {
                "env_id": {
                    "type": "integer"
                },
                "resource_ids": {
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "resource_type": {
                    "type": "string"
                }
            }
        },
        "web.BindResourceReq": {
            "type": "object",
            "required": [
                "env_id",
                "resource_id",
                "resource_type"
            ],
            "properties": {
                "env_id": {
                    "description": "环境ID",
                    "type": "integer"
                },
                "resource_id": {
                    "type": "integer"
                },
                "resource_type": {
                    "description": "instance/asset",
                    "type": "string"
                }
            }
        },
        "web.BindingVO": {
            "type": "object",
            "properties": {
                "bind_type": {
                    "type": "string"
                },
                "create_time": {
                    "type": "integer"
                },
                "env_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "node_id": {
                    "type": "integer"
                },
                "resource_id": {
                    "type": "integer"
                },
                "resource_type": {
                    "type": "string"
                },
                "rule_id": {
                    "type": "integer"
                }
            }
        },
        "web.CloudAccount": {
            "type": "object",
            "properties": {
                "access_key_id": {
                    "type": "string"
                },
                "access_key_secret": {
                    "description": "敏感信息，通常不返回",
                    "type": "string"
                },
                "asset_count": {
                    "type": "integer"
                },
                "config": {
                    "$ref": "#/definitions/web.CloudAccountConfigVO"
                },
                "create_time": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "environment": {
                    "type": "string"
                },
                "error_message": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "last_sync_time": {
                    "type": "string"
                },
                "last_test_time": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                },
                "regions": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "status": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                },
                "update_time": {
                    "type": "string"
                }
            }
        },
        "web.CloudAccountConfigVO": {
            "type": "object",
            "properties": {
                "enable_auto_sync": {
                    "type": "boolean"
                },
                "enable_cost_monitoring": {
                    "type": "boolean"
                },
                "read_only": {
                    "type": "boolean"
                },
                "show_sub_accounts": {
                    "type": "boolean"
                },
                "supported_asset_types": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "supported_regions": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "sync_interval": {
                    "type": "integer"
                }
            }
        },
        "web.CloudAsset": {
            "type": "object",
            "properties": {
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "asset_type": {
                    "type": "string"
                },
                "cost": {
                    "type": "number"
                },
                "create_time": {
                    "type": "string"
                },
                "discover_time": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "metadata": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.Tag"
                    }
                },
                "update_time": {
                    "type": "string"
                },
                "zone": {
                    "type": "string"
                }
            }
        },
        "web.CreateAssetReq": {
            "type": "object",
            "required": [
                "asset_id",
                "asset_name",
                "asset_type",
                "provider",
                "region"
            ],
            "properties": {
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "asset_type": {
                    "type": "string"
                },
                "cost": {
                    "type": "number"
                },
                "create_time": {
                    "type": "string"
                },
                "discover_time": {
                    "type": "string"
                },
                "metadata": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.Tag"
                    }
                },
                "update_time": {
                    "type": "string"
                },
                "zone": {
                    "type": "string"
                }
            }
        },
        "web.CreateCloudAccountReq": {
            "type": "object",
            "required": [
                "access_key_id",
                "access_key_secret",
                "environment",
                "name",
                "provider",
                "regions",
                "tenant_id"
            ],
            "properties": {
                "access_key_id": {
                    "type": "string",
                    "maxLength": 128,
                    "minLength": 16
                },
                "access_key_secret": {
                    "type": "string",
                    "maxLength": 256,
                    "minLength": 16
                },
                "config": {
                    "$ref": "#/definitions/web.CloudAccountConfigVO"
                },
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "environment": {
                    "type": "string"
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "provider": {
                    "type": "string"
                },
                "regions": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "type": "string"
                    }
                },
                "tenant_id": {
                    "type": "string"
                }
            }
        },
        "web.CreateEnvReq": {
            "type": "object",
            "required": [
                "code",
                "name"
            ],
            "properties": {
                "code": {
                    "type": "string"
                },
                "color": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                }
            }
        },
        "web.CreateFieldGroupReq": {
            "type": "object",
            "required": [
                "model_uid",
                "name"
            ],
            "properties": {
                "index": {
                    "type": "integer"
                },
                "model_uid": {
                    "type": "string"
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                }
            }
        },
        "web.CreateFieldReq": {
            "type": "object",
            "required": [
                "display_name",
                "field_name",
                "field_type",
                "field_uid",
                "model_uid"
            ],
            "properties": {
                "display": {
                    "description": "是否显示（默认true）",
                    "type": "boolean"
                },
                "display_name": {
                    "description": "显示名称",
                    "type": "string"
                },
                "field_name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "field_type": {
                    "type": "string"
                },
                "field_uid": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "group_id": {
                    "type": "integer"
                },
                "index": {
                    "type": "integer"
                },
                "link": {
                    "description": "是否为关联字段",
                    "type": "boolean"
                },
                "link_model": {
                    "description": "关联模型UID",
                    "type": "string"
                },
                "model_uid": {
                    "type": "string"
                },
                "option": {
                    "type": "string"
                },
                "required": {
                    "type": "boolean"
                },
                "secure": {
                    "type": "boolean"
                }
            }
        },
        "web.CreateFromGroupVO": {
            "type": "object",
            "required": [
                "group_id",
                "name"
            ],
            "properties": {
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "group_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                }
            }
        },
        "web.CreateMultiAssetsReq": {
            "type": "object",
            "required": [
                "assets"
            ],
            "properties": {
                "assets": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.CreateAssetReq"
                    }
                }
            }
        },
        "web.CreateNodeReq": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "description": {
                    "description": "描述",
                    "type": "string"
                },
                "name": {
                    "description": "节点名称",
                    "type": "string"
                },
                "order": {
                    "description": "排序",
                    "type": "integer"
                },
                "owner": {
                    "description": "负责人",
                    "type": "string"
                },
                "parent_id": {
                    "description": "父节点ID",
                    "type": "integer"
                },
                "tags": {
                    "description": "标签",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "team": {
                    "description": "团队",
                    "type": "string"
                },
                "uid": {
                    "description": "节点唯一标识",
                    "type": "string"
                }
            }
        },
        "web.CreateRuleReq": {
            "type": "object",
            "required": [
                "conditions",
                "env_id",
                "name",
                "node_id"
            ],
            "properties": {
                "conditions": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.RuleCondition"
                    }
                },
                "description": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "env_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "node_id": {
                    "type": "integer"
                },
                "priority": {
                    "type": "integer"
                }
            }
        },
        "web.CreateSyncTaskVO": {
            "type": "object",
            "required": [
                "cloud_account_id",
                "provider",
                "target_id",
                "target_type",
                "task_type"
            ],
            "properties": {
                "cloud_account_id": {
                    "type": "integer"
                },
                "provider": {
                    "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                },
                "target_id": {
                    "type": "integer"
                },
                "target_type": {
                    "$ref": "#/definitions/domain.SyncTargetType"
                },
                "task_type": {
                    "$ref": "#/definitions/domain.SyncTaskType"
                }
            }
        },
        "web.CreateTemplateVO": {
            "type": "object",
            "required": [
                "category",
                "cloud_platforms",
                "name",
                "tenant_id"
            ],
            "properties": {
                "category": {
                    "$ref": "#/definitions/domain.TemplateCategory"
                },
                "cloud_platforms": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                    }
                },
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "policies": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.PermissionPolicy"
                    }
                },
                "tenant_id": {
                    "type": "string"
                }
            }
        },
        "web.CreateTenantVO": {
            "type": "object",
            "required": [
                "id",
                "name"
            ],
            "properties": {
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "display_name": {
                    "type": "string",
                    "maxLength": 200
                },
                "id": {
                    "type": "string",
                    "maxLength": 50,
                    "minLength": 1
                },
                "metadata": {
                    "$ref": "#/definitions/domain.TenantMetadata"
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "settings": {
                    "$ref": "#/definitions/domain.TenantSettings"
                }
            }
        },
        "web.CreateUserGroupVO": {
            "type": "object",
            "required": [
                "cloud_platforms",
                "name",
                "tenant_id"
            ],
            "properties": {
                "cloud_platforms": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                    }
                },
                "description": {
                    "type": "string",
                    "maxLength": 500
                },
                "name": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                },
                "policies": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.PermissionPolicy"
                    }
                },
                "tenant_id": {
                    "type": "string"
                }
            }
        },
        "web.CreateUserVO": {
            "type": "object",
            "required": [
                "cloud_account_id",
                "tenant_id",
                "user_type",
                "username"
            ],
            "properties": {
                "cloud_account_id": {
                    "type": "integer"
                },
                "display_name": {
                    "type": "string",
                    "maxLength": 200
                },
                "email": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                },
                "user_groups": {
                    "description": "用户组ID列表",
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                },
                "user_type": {
                    "$ref": "#/definitions/domain.CloudUserType"
                },
                "username": {
                    "type": "string",
                    "maxLength": 100,
                    "minLength": 1
                }
            }
        },
        "web.DatabaseInstanceListResp": {
            "type": "object",
            "properties": {
                "instances": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.DatabaseInstanceVO"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "web.DatabaseInstanceVO": {
            "type": "object",
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "attributes": {
                    "type": "object",
                    "additionalProperties": true
                },
                "create_time": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "model_uid": {
                    "type": "string"
                },
                "provider": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "web.DiscoverAssetsReq": {
            "type": "object",
            "required": [
                "provider"
            ],
            "properties": {
                "asset_types": {
                    "description": "要发现的资源类型，为空则发现所有支持的类型",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "provider": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                }
            }
        },
        "web.EnvironmentVO": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "string"
                },
                "color": {
                    "type": "string"
                },
                "create_time": {
                    "type": "integer"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                },
                "status": {
                    "type": "integer"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "web.ErrorResponse": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer",
                    "example": 400
                },
                "msg": {
                    "type": "string",
                    "example": "租户ID不能为空"
                }
            }
        },
        "web.GenerateAuditReportVO": {
            "type": "object",
            "required": [
                "end_time",
                "start_time",
                "tenant_id"
            ],
            "properties": {
                "end_time": {
                    "type": "string"
                },
                "start_time": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                }
            }
        },
        "web.MatchInfo": {
            "type": "object",
            "properties": {
                "field": {
                    "description": "匹配的字段名",
                    "type": "string"
                },
                "label": {
                    "description": "字段显示名称",
                    "type": "string"
                },
                "value": {
                    "description": "匹配的字段值",
                    "type": "string"
                }
            }
        },
        "web.MenuItem": {
            "type": "object",
            "properties": {
                "children": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.MenuItem"
                    }
                },
                "icon": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                },
                "path": {
                    "type": "string"
                }
            }
        },
        "web.MoveNodeReq": {
            "type": "object",
            "properties": {
                "new_parent_id": {
                    "type": "integer"
                }
            }
        },
        "web.NodeVO": {
            "type": "object",
            "properties": {
                "create_time": {
                    "type": "integer"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "level": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "parent_id": {
                    "type": "integer"
                },
                "path": {
                    "type": "string"
                },
                "status": {
                    "type": "integer"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "team": {
                    "type": "string"
                },
                "uid": {
                    "type": "string"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "web.PageResult": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer"
                },
                "data": {},
                "msg": {
                    "type": "string"
                },
                "page": {
                    "type": "integer"
                },
                "size": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "web.Result": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer"
                },
                "data": {},
                "msg": {
                    "type": "string"
                }
            }
        },
        "web.RuleVO": {
            "type": "object",
            "properties": {
                "conditions": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.RuleCondition"
                    }
                },
                "create_time": {
                    "type": "integer"
                },
                "description": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "env_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "node_id": {
                    "type": "integer"
                },
                "priority": {
                    "type": "integer"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "web.SearchListResp": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.SearchResultVO"
                    }
                },
                "keyword": {
                    "description": "返回搜索关键词，方便前端高亮",
                    "type": "string"
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "web.SearchResultResponse": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer",
                    "example": 0
                },
                "data": {
                    "$ref": "#/definitions/web.SearchListResp"
                },
                "msg": {
                    "type": "string",
                    "example": "success"
                }
            }
        },
        "web.SearchResultVO": {
            "type": "object",
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "asset_type": {
                    "type": "string"
                },
                "attributes": {
                    "type": "object",
                    "additionalProperties": true
                },
                "create_time": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "matches": {
                    "description": "匹配信息，用于前端高亮",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.MatchInfo"
                    }
                },
                "provider": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "web.SubmitDiscoverAssetsTaskReq": {
            "type": "object",
            "required": [
                "provider"
            ],
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_types": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "provider": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                }
            }
        },
        "web.SubmitSyncAssetsTaskReq": {
            "type": "object",
            "required": [
                "provider"
            ],
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_types": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "provider": {
                    "type": "string"
                },
                "regions": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "web.SubmitTaskResp": {
            "type": "object",
            "properties": {
                "message": {
                    "type": "string"
                },
                "task_id": {
                    "type": "string"
                }
            }
        },
        "web.SyncAccountReq": {
            "description": "同步云账号资产的请求参数",
            "type": "object",
            "properties": {
                "asset_types": {
                    "description": "要同步的资源类型列表，支持: ecs, rds, oss, vpc 等\n为空则默认只同步 ecs",
                    "type": "array",
                    "items": {
                        "type": "string"
                    },
                    "example": [
                        "ecs",
                        "rds"
                    ]
                },
                "regions": {
                    "description": "要同步的地域列表，为空则同步账号配置的所有地域",
                    "type": "array",
                    "items": {
                        "type": "string"
                    },
                    "example": [
                        "cn-hangzhou",
                        "cn-shanghai"
                    ]
                }
            }
        },
        "web.SyncAssetsReq": {
            "type": "object",
            "required": [
                "account_id"
            ],
            "properties": {
                "account_id": {
                    "description": "云账号ID",
                    "type": "integer"
                },
                "asset_types": {
                    "description": "要同步的资源类型，为空则同步所有支持的类型",
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                }
            }
        },
        "web.SyncResult": {
            "description": "云资产同步操作的结果",
            "type": "object",
            "properties": {
                "message": {
                    "description": "同步结果消息",
                    "type": "string",
                    "example": "同步完成，共同步 10 个资产"
                },
                "start_time": {
                    "description": "同步开始时间",
                    "type": "string"
                },
                "status": {
                    "description": "同步状态: running, success, failed",
                    "type": "string",
                    "example": "success"
                },
                "sync_id": {
                    "description": "同步任务ID，格式: sync_{account_id}_{timestamp}",
                    "type": "string",
                    "example": "sync_1_1706000000"
                }
            }
        },
        "web.Tag": {
            "type": "object",
            "properties": {
                "key": {
                    "type": "string"
                },
                "value": {
                    "type": "string"
                }
            }
        },
        "web.TaskListResp": {
            "type": "object",
            "properties": {
                "tasks": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.TaskResp"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "web.TaskResp": {
            "type": "object",
            "properties": {
                "completed_at": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "string"
                },
                "duration": {
                    "type": "integer"
                },
                "error": {
                    "type": "string"
                },
                "id": {
                    "type": "string"
                },
                "message": {
                    "type": "string"
                },
                "params": {
                    "type": "object",
                    "additionalProperties": true
                },
                "progress": {
                    "type": "integer"
                },
                "result": {
                    "type": "object",
                    "additionalProperties": true
                },
                "started_at": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "type": {
                    "type": "string"
                }
            }
        },
        "web.TreeNodeVO": {
            "type": "object",
            "properties": {
                "children": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.TreeNodeVO"
                    }
                },
                "create_time": {
                    "type": "integer"
                },
                "description": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "level": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "parent_id": {
                    "type": "integer"
                },
                "path": {
                    "type": "string"
                },
                "resource_count": {
                    "type": "integer"
                },
                "status": {
                    "type": "integer"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "team": {
                    "type": "string"
                },
                "uid": {
                    "type": "string"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "web.UnifiedAssetListResp": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.UnifiedAssetVO"
                    }
                },
                "total": {
                    "type": "integer"
                }
            }
        },
        "web.UnifiedAssetVO": {
            "type": "object",
            "properties": {
                "account_id": {
                    "type": "integer"
                },
                "asset_id": {
                    "type": "string"
                },
                "asset_name": {
                    "type": "string"
                },
                "asset_type": {
                    "type": "string"
                },
                "attributes": {
                    "type": "object",
                    "additionalProperties": true
                },
                "create_time": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "provider": {
                    "type": "string"
                },
                "region": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "tenant_id": {
                    "type": "string"
                },
                "update_time": {
                    "type": "integer"
                }
            }
        },
        "web.UpdateAssetReq": {
            "type": "object",
            "properties": {
                "asset_name": {
                    "type": "string"
                },
                "cost": {
                    "type": "number"
                },
                "metadata": {
                    "type": "string"
                },
                "status": {
                    "type": "string"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/web.Tag"
                    }
                },
                "update_time": {
                    "type": "string"
                }
            }
        },
        "web.UpdateCloudAccountReq": {
            "type": "object",
            "properties": {
                "access_key_id": {
                    "type": "string"
                },
                "access_key_secret": {
                    "type": "string"
                },
                "config": {
                    "$ref": "#/definitions/web.CloudAccountConfigVO"
                },
                "description": {
                    "type": "string"
                },
                "environment": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "regions": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "tenant_id": {
                    "type": "string"
                }
            }
        },
        "web.UpdateEnvReq": {
            "type": "object",
            "required": [
                "code",
                "name"
            ],
            "properties": {
                "code": {
                    "type": "string"
                },
                "color": {
                    "type": "string"
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                },
                "status": {
                    "type": "integer"
                }
            }
        },
        "web.UpdateFieldGroupReq": {
            "type": "object",
            "properties": {
                "index": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                }
            }
        },
        "web.UpdateFieldReq": {
            "type": "object",
            "properties": {
                "display": {
                    "type": "boolean"
                },
                "display_name": {
                    "type": "string"
                },
                "field_name": {
                    "type": "string"
                },
                "field_type": {
                    "type": "string"
                },
                "group_id": {
                    "type": "integer"
                },
                "index": {
                    "type": "integer"
                },
                "link": {
                    "description": "是否为关联字段",
                    "type": "boolean"
                },
                "link_model": {
                    "description": "关联模型UID",
                    "type": "string"
                },
                "option": {
                    "type": "string"
                },
                "required": {
                    "type": "boolean"
                },
                "secure": {
                    "type": "boolean"
                }
            }
        },
        "web.UpdateNodeReq": {
            "type": "object",
            "required": [
                "name"
            ],
            "properties": {
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "order": {
                    "type": "integer"
                },
                "owner": {
                    "type": "string"
                },
                "status": {
                    "type": "integer"
                },
                "tags": {
                    "type": "array",
                    "items": {
                        "type": "string"
                    }
                },
                "team": {
                    "type": "string"
                },
                "uid": {
                    "type": "string"
                }
            }
        },
        "web.UpdatePoliciesVO": {
            "type": "object",
            "required": [
                "policies"
            ],
            "properties": {
                "policies": {
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "$ref": "#/definitions/domain.PermissionPolicy"
                    }
                }
            }
        },
        "web.UpdateRuleReq": {
            "type": "object",
            "required": [
                "conditions",
                "env_id",
                "name",
                "node_id"
            ],
            "properties": {
                "conditions": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.RuleCondition"
                    }
                },
                "description": {
                    "type": "string"
                },
                "enabled": {
                    "type": "boolean"
                },
                "env_id": {
                    "type": "integer"
                },
                "name": {
                    "type": "string"
                },
                "node_id": {
                    "type": "integer"
                },
                "priority": {
                    "type": "integer"
                }
            }
        },
        "web.UpdateTemplateVO": {
            "type": "object",
            "properties": {
                "cloud_platforms": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                    }
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "policies": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.PermissionPolicy"
                    }
                }
            }
        },
        "web.UpdateTenantVO": {
            "type": "object",
            "properties": {
                "description": {
                    "type": "string"
                },
                "display_name": {
                    "type": "string"
                },
                "metadata": {
                    "$ref": "#/definitions/domain.TenantMetadata"
                },
                "name": {
                    "type": "string"
                },
                "settings": {
                    "$ref": "#/definitions/domain.TenantSettings"
                },
                "status": {
                    "$ref": "#/definitions/domain.TenantStatus"
                }
            }
        },
        "web.UpdateUserGroupVO": {
            "type": "object",
            "properties": {
                "cloud_platforms": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/github_com_Havens-blog_e-cam-service_internal_shared_domain.CloudProvider"
                    }
                },
                "description": {
                    "type": "string"
                },
                "name": {
                    "type": "string"
                },
                "policies": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/domain.PermissionPolicy"
                    }
                }
            }
        },
        "web.UpdateUserVO": {
            "type": "object",
            "properties": {
                "display_name": {
                    "type": "string"
                },
                "email": {
                    "type": "string"
                },
                "status": {
                    "$ref": "#/definitions/domain.CloudUserStatus"
                },
                "user_groups": {
                    "description": "用户组ID列表",
                    "type": "array",
                    "items": {
                        "type": "integer"
                    }
                }
            }
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "1.0",
	Host:             "localhost:8001",
	BasePath:         "/api/v1",
	Schemes:          []string{"http", "https"},
	Title:            "E-CAM Service API",
	Description:      "企业云资产管理服务 API 文档",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
