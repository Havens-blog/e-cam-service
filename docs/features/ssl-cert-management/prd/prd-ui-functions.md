...
- 只读查看者仅可见"到期看板"一级菜单；证书台账（/certs）与变更管理（/certs/changes）菜单对只读角色不展示，直接访问路由被前端拦截并提示无权限，对应接口由 EIAM 同步拦截
...
6. 仅指纹登记证书行点击"补传私钥" → 上传私钥 → 通过匹配校验后证书升级为完整托管（托管状态即时更新）
7. 点击"发起更换"（完整托管证书）→ 进入变更向导（UF-4）
...
| 导入表单 | Form | 用户输入 | PEM 文件 + 私钥文件（可选）+ 预期域名（可选，仅提示性比对） |
| 补传私钥表单 | Form | 用户输入 | 私钥文件（仅指纹登记证书可用，匹配校验通过后升级完整托管） |
...
| State | Display | Trigger |
|-------|---------|---------|
| loading | 骨架屏 | 引用关系区块首次加载 |
| error | 错误提示 + 重试 | 扫描结果服务异常 |
...
2. 选择旧证书 + 新证书 → 系统校验扫描新鲜度（超期阻断并引导扫描）→ 生成清单（逐项资源、计划动作、原证书 ID、覆盖边界声明、SAN 预检结果）；华为云/AWS/Azure 引用点与 GitOps/控制器管理目标单列为"不可执行项/不可自动变更"分区，标注原因与出路（二期或手工 / 走其管理链路），不参与自动执行
3. 用户确认清单（可勾选同单分批，单批 ≤ 总量一半）→ 执行；首批执行且验证通过后展示"执行剩余批"入口，剩余批同样需人工确认
4. 执行中：逐项状态实时刷新（成功/失败/执行中/限流重试中）；出现失败项后，已成功项出现"回滚"入口（失败项引用未被改动，无需回滚）
5. 执行完成进入验证窗口：展示窗口倒计时与逐项验证状态（达标/差异-变更关联告警），并保留成功项回滚入口；窗口关闭仍未达标项转常规差异告警并记入报告"未达标清单"
6. 查看变更报告：清单、逐项结果、回滚状态、验证结论、孤儿证书补偿清理结果（逐项清理成功/失败）
7. 回滚保护期内旧证书行显示保护期标记
...
| 变更清单 | List | 清单生成服务 | 资源、计划动作、原证书 ID、预检结果；不可执行项/不可自动变更标记及原因 |
| 执行进度 | Stream/Poll | 执行通道 | 逐项状态（成功/失败/执行中/限流重试中）与错误信息 |
| 验证状态 | List | TLS 探测 | 窗口内预期终态比对结果、未达标清单 |
| 变更报告 | Report | 变更服务 | 清单、逐项结果、回滚状态、验证结论、孤儿证书补偿清理结果 |

### States

| State | Display | Trigger |
|-------|---------|---------|
| loading | 骨架屏 | 列表/清单/报告首次加载 |
| empty | 空态引导（"新建变更"入口） | 无变更单 |
| error | 错误提示 + 重试 | 变更服务异常 |
| 清单被阻断 | 阻断原因（扫描超期/SAN 不满足/旧证书已有在途变更单）+ 引导动作 | 新鲜度/预检/互斥校验失败 |
| 执行中 | 逐项进度（含限流重试中）+ 出现失败项后成功项可回滚 | 变更执行 |
| 验证中 | 窗口倒计时 + 逐项验证状态 + 成功项回滚入口 | 进入验证窗口 |
| 部分完成 | 报告入口 + 成功项回滚入口 + 回滚保护期剩余 | 窗口关闭且存在未达标/失败项 |
| 回滚失败 | 告警横幅 + 转人工提示 | 回滚执行失败 |
| 已完成 | 报告入口 + 回滚保护期剩余 | 窗口关闭且达标 |

### Validation Rules

- 清单生成前置校验：扫描新鲜度、SAN ⊇ 目标域名、同一旧证书无在途变更单（互斥），任一不满足阻断
- 分批为同一变更单内分批：单批不超过清单总量 50%；首批执行且验证通过后可执行剩余批，剩余批同样需人工确认
- 回滚入口仅在执行中（出现失败项后）、验证中、部分完成状态可用，且仅作用于执行成功项
...
| State | Display | Trigger |
|-------|---------|---------|
| empty | 空态引导（提示首次配置接收人与豁免清单） | 无任何配置 |
| populated | 配置表单 | 有配置 |
