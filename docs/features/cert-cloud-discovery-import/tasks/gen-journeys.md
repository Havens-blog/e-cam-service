...
dependencies: ["T-clean-code", "3", "4", "5", "6", "7", "8", "1", "2"]
type: "test.gen-journeys"
surface-key: ""
surface-type: ""
---

Generate test Journey documents for the cert-cloud-discovery-import feature.
Mode: quick
...
- **Breakdown mode**: Read PRD user stories from `docs/features/cert-cloud-discovery-import/prd/prd-user-stories.md` and functional specs from `docs/features/cert-cloud-discovery-import/prd/prd-spec.md`. These are the primary input sources.
- **Quick mode**: Read the proposal from `docs/proposals/cert-cloud-discovery-import/proposal.md`. Extract Key Scenarios as Journey candidates. If the proposal lacks `scope` or `success criteria` sections, abort the task with a diagnostic message — Journey generation requires these minimum inputs.
...
5. **Generate Files**: Output one `journey.md` per Journey to `docs/features/cert-cloud-discovery-import/testing/<journey-name>/journey.md`
...
- **If AUTO_COMMIT=true**: Skip the user review-and-approval step. After validation passes, directly commit all generated Journey files:
  ```bash
  git add docs/features/cert-cloud-discovery-import/testing/
  git commit -m "docs: generate journeys for cert-cloud-discovery-import"
  ```
...
- [ ] At least 1 Journey file generated under `docs/features/cert-cloud-discovery-import/testing/`
