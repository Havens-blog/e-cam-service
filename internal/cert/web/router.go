...
// 3.6 引用关系 + 4.5 看板与全局配置 + 5.11 变更管理 + cert-cloud-discovery-import
// 任务 3 发现导入查询）。鉴权由全局中间件链（ioc/gin.go EcmdbAuthMiddleware）
// 承接，此处不重复挂载；settings 与变更面端点级 RequireRoles 角色门卫
// （7.2 EIAM 全量接线）。
// reference/discovery/dashboard/settings/change 先于 ledger 注册：/reverse、
// /discovery、/dashboard、/settings、/changes 静态段须先于 /:id 进入路由树。
func RegisterRoutes(
	server *gin.Engine,
	h *CertHandler,
