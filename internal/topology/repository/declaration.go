...
type DeclarationRepository interface {
	...
	FindBySource(ctx context.Context, tenantID int64, source string) ([]domain.LinkDeclaration, error)
	// FindAll 查询租户下所有声明数据
	FindAll(ctx context.Context, tenantID int64) ([]domain.LinkDeclaration, error)
...
func (r *declarationRepository) FindBySource(ctx context.Context, tenantID int64, source string) ([]domain.LinkDeclaration, error) {
	return r.dao.FindBySource(ctx, tenantID, source)
}

func (r *declarationRepository) FindAll(ctx context.Context, tenantID int64) ([]domain.LinkDeclaration, error) {
	return r.dao.FindAll(ctx, tenantID)
}

func (r *declarationRepository) DeleteBySource(ctx context.Context, tenantID int64, source string) (int64, error) {
