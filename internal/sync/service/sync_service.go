...
func (s *SyncService) SyncECSInstances(ctx context.Context, account *shareddomain.CloudAccount, regions []string) (*domain.SyncResult, error) {
	...
	if len(regions) == 0 {
		...
		if ecsAdapter == nil {
			return nil, fmt.Errorf("ECS适配器不可用")
		}
		allRegions, err := ecsAdapter.GetRegions(ctx)
		if err != nil {
...
func (s *SyncService) syncRegionECSInstances(
	...
	// 从云厂商获取实例列表
	instances, err := ecsAdapter.ListInstances(ctx, region)
	if err != nil {
