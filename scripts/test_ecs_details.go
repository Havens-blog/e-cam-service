...
func main() {
	...
	fmt.Println("\n【5. 获取实例监控数据】")
	endTime := time.Now()
	startTime := endTime.Add(-1 * time.Hour) // 最近1小时
	...
	if err != nil {
		...
		if len(monitorData.DataPoints) > 0 {
			...
			for i := 0; i < count; i++ {
				...
				fmt.Printf("      公网入流量:   %d KB/s\n", dp.InternetBandwidthIn)
				fmt.Printf("      公网出流量:   %d KB/s\n", dp.InternetBandwidthOut)
				fmt.Printf("      内网入流量:   %d KB/s\n", dp.IntranetBandwidthIn)
				fmt.Printf("      内网出流量:   %d KB/s\n", dp.IntranetBandwidthOut)
			}
		}
	}
